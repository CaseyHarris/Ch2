...
#for (i in wq_names$wq_names) {
  ...
  #mcmc_summary <- read.table(paste0("C:/Users/cshar/OneDrive - University of Florida/Online_diss_files/Ch2/Large files/Results/MCMC JAGS/", title_use, ".csv", sep=""), header=TRUE)
  mcmc_summary <- read.table(paste0("/blue/carpena/caseyharris/Ch2/JAGS/MCMC/", title_use, ".csv", sep=""), header=TRUE)
  #cal_val <- read.table(paste0("C:/Users/cshar/OneDrive - University of Florida/Online_diss_files/Ch2/Large files/Results/Cal_val JAGS/", title_use, ".csv", sep=""), header=TRUE)
  ...
  wq_flow_past1 <- wq_flow_past %>%
    mutate(date=as.Date(Date),
           ...
           Realiz_num=0,
           scenario="USGS") %>%
    select(title, scenario, date, Chang_item, Realiz_num, Q_cfs_log_round, Q_cfs_log_round_edit, 8:ncol(wq_flow_past))
  ...
  wq_flow_jason1 <- wq_flow_jason %>%
    mutate(date=as.Date(date)) %>%
    select(title, scenario, date, Chang_item, Realiz_num, Q_cfs_log_round, Q_cfs_log_round_edit, 8:ncol(wq_flow_jason))
  ...
  #wq_list <- seq(from=min_wq, to=max_wq, length.out=1000) #modeled range, may include too large of extremes?
  wq_list <- seq(from=max_min1$value[max_min1$name=="min_wq_2010"], to=max_min1$value[max_min1$name=="max_wq_2010"], length.out=1000) #may still include too large of extremes
  l=100
  m=2
  q=median(wq_list)
  exceed_df <- data.frame()
  #for (l in unique(both1$Chang_item)[1:2]) {
  for (l in unique(both1$Chang_item)) {
    #for (m in 1:5) {
    for (m in 1:length(mcmc_summary$split)) {
      ...
      for (q in wq_list) {
        ...
        sub1 <- sub %>%
          mutate(above_thresh = case_when(sub[,m+7]>q ~ 1,
                                          TRUE ~ 0))#%>%
