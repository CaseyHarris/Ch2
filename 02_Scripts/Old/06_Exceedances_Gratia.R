# Probabilities of water quality changes

#This script joins the water quality estimates for each flow 
#to the actual dates in the past/future flow datasets,
#and then begins to summarize the results

#HAVEN'T STARTED YET, CURRENTLY JUST COPY-PASTED FROM JAGS VERSION
...
#List of water quality datasets to use
...
#List of relevant max/min values of water quality and flow
...
#Reduce climate model list to only the models we are interested in
...
models_to_use <- model_names2 %>%
  filter(str_detect(model, "Hargreaves") | model=="USGS") #only Hargreaves and USGS
rm(add_names)
rm(model_names)
rm(model_names1)
rm(model_names2)
...
#         .packages=c('tidyverse', 'data.table', 'readxl')) %dopar% {
          ...
          y_all <- read.csv(paste("C:/Users/cshar/OneDrive - University of Florida/Online_diss_files/Ch2/Large files/Results/WQ per flow cal val/", title_use, ".csv", sep=""))
          #y_all <- read.table(paste("C:/Users/cshar/OneDrive - University of Florida/Online_diss_files/Ch2/Large files/Results/WQ per flow cal val/", title_use, ".csv", sep=""), header=TRUE)
          #y_all <- read.table(paste("/blue/carpena/caseyharris/Ch2/Results/WQ per flow wide/", title_use, ".csv", sep=""), header=TRUE)
