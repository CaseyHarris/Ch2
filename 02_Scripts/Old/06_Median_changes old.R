...
wq_names <- data.frame(wq_names) %>%
  ...
  filter(str_detect(wq_names, "Fluoride|Nitrate|Orthophosphate|Specific|carbon|phosphorus|color|Turbidity"))
...
#for (i in 1:length(wq_names$wq_names)) {
          ...
          both_y <- both_y %>%
            ...
            #          Chang_item==677 |
            #          Chang_item==680) %>%
            select(Chang_item, month, in_out, s1:s100) %>% #change to s1000?
            pivot_longer(cols=s1:s100, names_to="sim", values_to="val") #%>%
          ...
          #ggsave(paste("/blue/carpena/caseyharris/Ch2/Results/Tileplots/", title_use, " future1 pos prob.png"), height=6, width=12)
          ...
          if (i==1) {
            #write.table(sel_vals_summary1, paste("C:/Users/cshar/OneDrive - University of Florida/Online_diss_files/Ch2/Large files/Results/Exceedance in prog/", title_use, ".csv", sep=""), row.names=FALSE, col.names=TRUE)
            write.table(sel_vals_summary1, paste("/blue/carpena/caseyharris/Ch2/Results/Tileplots/", title_use, ".csv", sep=""), row.names=FALSE, col.names=TRUE)
          } else {
            ...
            write.table(sel_vals_summary1, paste("/blue/carpena/caseyharris/Ch2/Results/Tileplots/", title_use, ".csv", sep=""), row.names=FALSE, col.names=FALSE, append=TRUE)
          }
