...
#The table is used in further scripts to know when
#we are making water quality estimates outside of the 
#historically observed range of flow.
...
for (i in wq_names_df$wq_names) {
  ...
  if (str_detect(title, "Alafia")) {
    flow0 <- flow_Alaf_Hills %>%
      filter(site=="Alafia" & Date<as.Date("2020-01-01")) %>%
      mutate(source="USGS") %>%
    ...
    flow0 <- flow_Alaf_Hills %>%
      filter(site=="Hillsborough" & Date<as.Date("2020-01-01")) %>%
      mutate(source="USGS") %>%
      select(source, Date, Q_cfs, Q_cfs_log_round)
    site <- "Hillsborough"
    flow1 <- flow_Jason1 %>%
  ...
  #max 1989-2012, 2010-2019, future
  wq_flow1 <- wq_flow %>%
  ...
  max_flow_2010_2019 <- max(wq_flow1$Q_cfs_log_round[wq_flow1$source=="USGS" & wq_flow1$Date>=as.Date("2010-01-01") & wq_flow1$Date<as.Date("2020-01-01")])
  min_flow_2010_2019 <- min(wq_flow1$Q_cfs_log_round[wq_flow1$source=="USGS" & wq_flow1$Date>=as.Date("2010-01-01") & wq_flow1$Date<as.Date("2020-01-01")])
  ...
  max_wq_2010_2019 <- max(wq_flow1$ConcAve[wq_flow1$source=="USGS" & !is.na(wq_flow1$ConcAve) & wq_flow1$Date>=as.Date("2010-01-01") & wq_flow1$Date<as.Date("2020-01-01")])
  min_wq_2010_2019 <- min(wq_flow1$ConcAve[wq_flow1$source=="USGS" & !is.na(wq_flow1$ConcAve) & wq_flow1$Date>=as.Date("2010-01-01") & wq_flow1$Date<as.Date("2020-01-01")])
  ...
  max_flow_obs_wq_2010_2019 <- max(wq_flow1$Q_cfs_log_round[wq_flow1$source=="USGS" & !is.na(wq_flow1$ConcAve) & wq_flow1$Date>=as.Date("2010-01-01") & wq_flow1$Date<as.Date("2020-01-01")])
  min_flow_obs_wq_2010_2019 <- min(wq_flow1$Q_cfs_log_round[wq_flow1$source=="USGS" & !is.na(wq_flow1$ConcAve) & wq_flow1$Date>=as.Date("2010-01-01") & wq_flow1$Date<as.Date("2020-01-01")])
  ...
  max_min_new <- data.frame("title"=title, "site"=site, 
                            ...
                            "max_flow_2010_2019" = max_flow_2010_2019,
                            "min_flow_2010_2019" = min_flow_2010_2019,
                            ...
                            "max_wq_2010_2019" = max_wq_2010_2019,
                            "min_wq_2010_2019" = min_wq_2010_2019,
                            "max_flow_obs_wq_1989_2012" = max_flow_obs_wq_1989_2012,
                            "min_flow_obs_wq_1989_2012" = min_flow_obs_wq_1989_2012,
                            "max_flow_obs_wq_2010_2019" = max_flow_obs_wq_2010_2019,
                            "min_flow_obs_wq_2010_2019" = min_flow_obs_wq_2010_2019)
