...
foreach(i = 1:length(wq_names$wq_names),
  ...
  min_wq <- both1 %>%
              select(-scenario, -date, -Chang_item, -Realiz_num, -Q_cfs_log_round, -Q_cfs_log_round_edit) %>%
              summarise(across(everything(), min))
  min_wq <- min_wq %>%
              rowwise() %>%
              mutate(min = min(c_across(8:ncol(min_wq)))) %>%
              select(min)
  ...
  max_wq <- max_wq %>%
              rowwise() %>%
              mutate(max = max(c_across(8:ncol(max_wq)))) %>%
              select(max)
  ...
  #and estimate exceedances for each wq level in the wq_list
              ...
              k=1
              q=median(wq_list)
              for (j in unique(both1$Chang_item)) {
              ...
              } #around 12 hrs per wq parameter (10 minutes per Chang_item)
            rm(sub)
            rm(sub1)
            rm(sub2)
            gc()
